...
const UpdateJasa = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:4000/api/v1/jasas/view/jasaid/" + id)
            .then((res) => {
                ...
                setNamaJasa(res.namaJasa)
                setStartingPrice(res.startingPrice)
                setDescJasa(res.descJasa)
                setCatJasa(res.catJasa)
    ...
    const onUpload = () => {
        ...
        setImagePreview(URL.createObjectURL(myNewCroppedFile))
        setIsClose(true)
    }
        ...
        <div className="create-jasa">
            ...
            {isClose && data && dataPenjasa && (
                <div className="form">
                    ...
                    <button onClick={() => setUpdate(true)}>Update Jasa</button>
                </div>
            )}
